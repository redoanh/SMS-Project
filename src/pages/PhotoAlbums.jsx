...
const useFetchAlbums = (url) => {
        ...
        axios.get(url)
            .then(response => {
                if (isMounted && response.data.success) {
                    ...
                    setRawAlbums(response.data.data);
                }
            })
            .catch(err => {
                if (isMounted) setError("Could not synchronize campus gallery assets.");
                console.error(err);
            })
            .finally(() => {
    ...
    return { rawAlbums, loading, error };
};

function PhotoAlbums() {
   
    const { rawAlbums, loading, error } = useFetchAlbums('http://127.0.0.1:8000/api/albums');
    
    const [activeAlbumKey, setActiveAlbumKey] = useState('All');
    const [lightboxImage, setLightboxImage] = useState(null);
    const [structuredAlbums, setStructuredAlbums] = useState({});

    useEffect(() => {
        if (rawAlbums.length > 0) {
            const grouped = {};
            
            rawAlbums.forEach(item => {
               
                const category = item.title || "General Gallery";
                if (!grouped[category]) {
                    grouped[category] = [];
                }
                grouped[category].push({
                    id: item.id,
                    title: item.title,
                    description: item.description || "Campus Event Photo",
                    url: item.url,
                    dimensions: item.dimensions || "1200 x 800 (HD)",
                    uploadDate: item.upload_date || "Recent"
                });
            });

            setStructuredAlbums(grouped);
        }
    }, [rawAlbums]);

    const allPhotos = rawAlbums.map(item => ({
        id: item.id,
        title: item.title,
    ...
    const visiblePhotos = activeAlbumKey === 'All' 
        ? allPhotos 
        : (structuredAlbums[activeAlbumKey] || []);
            ...
            <div className="max-w-[1200px] mx-auto space-y-8">
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                    <div className="bg-gradient-to-r from-[#1e3a8a] to-[#3b82f6] text-white p-8 text-center">
                        ...
                        <p className="text-blue-100 font-medium">
                            {loading ? 'Compiling campus history...' : `Browsing ${Object.keys(structuredAlbums).length} Event Albums (${allPhotos.length} Total Captures)`}
                        </p>
                    </div>
                </div>
                ...
                {loading && (
                    <div className="flex flex-col items-center justify-center p-12 bg-white rounded-2xl border border-gray-100 shadow-md">
                        ...
                        <p className="text-gray-500 font-medium">Streaming archive visual nodes from Laravel backend...</p>
                    </div>
                )}
                ...
                {!loading && !error && (
                    <>
                        <div className="flex flex-wrap justify-center gap-2 bg-white p-3 rounded-xl shadow-md border border-gray-100">
                            ...
                            {Object.keys(structuredAlbums).map((albumName) => (
                                <button
                                    key={albumName}
                                ...
                                >
                                    📁 {albumName} ({structuredAlbums[albumName].length})
                                </button>
                            ))}
                        </div>
                            ...
                            {visiblePhotos.map((photo, index) => (
                                ...
                                >
                                    ...
                                    <div className="p-4 bg-white flex-1 flex flex-col justify-between">
                                        <div>
                                            <h3 className="font-bold text-gray-800 text-sm mb-1 group-hover:text-blue-600 transition text-truncate">
                                                {photo.title}
                                            </h3>
